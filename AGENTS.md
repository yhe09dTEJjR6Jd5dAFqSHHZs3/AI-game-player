...
兼容性：适配任意硬件配置的Windows 11，不考虑多显示器，不考虑其他操作系统。
...
越美观越好。
实时显示以下信息：
截图频率、窗口A的名称/尺寸/画面/是否可见且完整；
CPU、内存、GPU、显存占用率、按钮、数值识别列表；
...
遮挡处理：若窗口A不可见/被遮挡/有一部分在电脑屏幕外，暂停记录，直到窗口A再次可见且完整（不考虑窗口A在不在前台）。
...
遮挡处理：若窗口A不可见/被遮挡/有一部分在电脑屏幕外，暂停输出和记录，直到窗口A再次可见且完整（不考虑窗口A在不在前台）。
