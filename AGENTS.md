角色设定：你是一位精通计算机视觉、深度学习（PyTorch）及GUI开发的资深Python工程师。
任务目标：编写一个单文件、无注释的Python脚本，实现一个通用游戏AI。
...
严禁要求用户手动安装除Python库以外的任何外部软件（如Tesseract等）。
所有生成文件（模型、数据）仅限于Desktop\GameAI及其子文件夹。
...
越美观越好。
功能模块布局：
顶部状态栏：显示截图频率、窗口A状态（名称/尺寸/是否可见且完整）。
右侧控制区：包含硬件监控、操作按钮组、数值识别列表。
中部预览区：居中显示窗口A实时画面。
...
列表设置：用户可对列表中的每个数值设置属性：越高越好/越低越好/变化小好/变化大好/无关/识别错误。需包含防误触的解锁/锁定功能。
...
阶段 C：训练模式
行为：AI接管控制，在窗口A上输出鼠标操作。
数据记录：持续记录AI的鼠标操作及画面到经验池（需标记数据来源为AI，与用户操作区分）。
AI目标：基于学习模式的数据，理解控件位置、形态及用法。
中断机制：检测到用户键盘操作 -> 停止AI操作 -> 切换回学习模式（不停止记录，仅切换操作来源）。
遮挡处理：同学习模式，窗口被遮挡或不可见时，暂停输出和记录。
