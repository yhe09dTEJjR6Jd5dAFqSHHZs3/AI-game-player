...
兼容性：适配任意硬件配置的Windows 11，不考虑其他操作系统。
...
1. 灵活
基于CPU、内存、GPU、显存的实时占用率，动态调整截图频率（0Hz~120Hz）和训练模式期间的AI推理强度。
安全退出：用户按下ESC键时，立即终止程序。
窗口适应性：支持用户在运行期间任意调整目标窗口（窗口A）及主程序窗口的大小和位置。

3. 主程序窗口
越美观越好。
实时显示以下信息：
截图频率，窗口A的名称/尺寸/预览画面/是否可见且完整（窗口A画面预览功能不受可见性和完整性影响）；
CPU、内存、GPU、显存占用率，DPI，按钮，数值识别列表；
...
阶段B：学习模式
行为：持续记录用户在窗口A上的鼠标操作/窗口A画面/声音，存入经验池（不记录键盘）。
遮挡处理：若窗口A不可见/被遮挡/有一部分在电脑屏幕外，暂停向经验池中记录数据，直到窗口A再次可见且完整（不考虑窗口A在不在前台）。
模式切换：检测到全局无键鼠操作持续10秒+窗口A可见且完整+没有在识别+没有在优化 -> 切换至训练模式。
...
数据记录：持续记录AI在窗口A上的鼠标操作/窗口A画面/声音，存入经验池（同样也不记录键盘，但需标记鼠标数据来源为AI，与用户操作区分）。
中断机制：检测到键盘操作 -> 停止AI操作 -> 切换回学习模式（不停止记录，仅切换操作来源）。
遮挡处理：若窗口A不可见/被遮挡/有一部分在电脑屏幕外，暂停AI输出，暂停向经验池中记录数据，直到窗口A再次可见且完整（不考虑窗口A在不在前台）。
