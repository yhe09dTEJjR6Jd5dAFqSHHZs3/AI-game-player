...
学习模式→空格→训练模式→检测到除ESC以外的任意物理按键信号，立即切断AI控制权并回退至学习模式；
训练模式→ESC结束运行；
...
对CPU占用率、内存占用率、GPU占用率、显存占用率、磁盘读写吞吐量、磁盘队列深度、磁盘响应时间、GPU温度、GPU功耗、风扇转速、推理延迟、数据采集循环耗时、OCR耗时、经验池填充率、截屏帧率、截屏延迟进行毫秒级监控，并建立优雅降级机制：资源负载接近饱和阈值时，采取主动降级措施，自动动态调整计算精度或挂起非关键线程。
