...
训练模式：AI根据实时数据，输出鼠标操作，同时记录数据到经验池（鼠标来源：AI）；
...
默认文件路径：C:\Program Files\Tesseract-OCR
用户划分的区域内需要识别的内容一定是非负整数。
如果识别成功，直接更新为整数；如果识别失败（比如数字闪烁），保持上一帧的数值不变，不再进行数学拟合。

要求：
简体中文界面。
仅用一个无注释的Python脚本实现所有功能。
适配任意操作系统、CPU、内存、GPU、显存、显示器分辨率、缩放、磁盘可用空间。
